...
package org.gibello.zql.ast.data;
...
public class ZEval {
	...
	public boolean eval(ZTuple tuple, ZExp exp) throws SQLException {
		...
		if (!(exp instanceof ZExpression))
			throw new SQLException(
					"ZEval.eval(): only expressions are supported");
		...
		if (op.equals("AND")) {
			...
			ZExpression newexp = new ZExpression("AND", new ZExpression(">=",
					pred.getOperand(0), pred.getOperand(1)), new ZExpression(
			...
			for (int i = 1; i < pred.nbOperands(); i++) {
				newexp.addOperand(new ZExpression("=", pred.getOperand(0), pred
						.getOperand(i)));
			}
	...
	double evalCmp(ZTuple tuple, Vector operands) throws SQLException {
		...
		if (o1 instanceof Number && o2 instanceof Number) {
			...
			throw new SQLException("ZEval.evalCmp(): can't compare ("
	...
	private boolean evalLike(ZTuple tuple, Vector operands) throws SQLException {
		...
		if ((o1 instanceof String) && (o2 instanceof String)) {
			...
			throw new SQLException(
					"ZEval.evalLike(): LIKE can only compare strings");
		}
	...
	double evalNumericExp(ZTuple tuple, ZExpression exp) throws SQLException {
		...
		if (!(o1 instanceof Double))
			throw new SQLException(
					"ZEval.evalNumericExp(): expression not numeric");
		...
		if (op.equals("+")) {
			...
			throw new SQLException("ZEval.evalNumericExp(): Unknown operator "
					+ op);
		}
	}
	...
	public Object evalExpValue(ZTuple tuple, ZExp exp) throws SQLException {
		...
		if (exp instanceof ZConstant) {
			...
			case ZConstant.COLUMNNAME:
				...
				if (o1 == null)
					throw new SQLException(
							"ZEval.evalExpValue(): unknown column "
									+ c.getValue());
				try {
